...
RUNFILE=${BASEDIR}/scripts/calc_mse_e.py

# Run the jobs
OUTDIR=${BASEDIR}/data/${EXP}_pp/${GRANULARITY}
mkdir -p ${OUTDIR}/mse_e_latlon
sbatch <<EOF
#!/bin/bash
...
#SBATCH --job-name="calc_mse_e"
#SBATCH --output=LOG/calc_mse_e.%j.out
#SBATCH --error=LOG/calc_mse_e.%j.out
#SBATCH --export=ALL
