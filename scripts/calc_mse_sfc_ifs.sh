...
RUNFILE=${BASEDIR}/scripts/calc_mse_sfc_ifs.py

# Run the jobs
OUTDIR=${BASEDIR}/data/${EXP}_pp/${GRANULARITY}
mkdir -p ${OUTDIR}/mse_sfc_latlon
sbatch <<EOF
#!/bin/bash
...
#SBATCH --job-name="calc_mse_sfc_ifs"
#SBATCH --output=LOG/calc_mse_sfc_ifs.%j.out
#SBATCH --error=LOG/calc_mse_sfc_ifs.%j.out
#SBATCH --export=ALL
