...
RUNFILE=${BASEDIR}/scripts/calc_sfcwind_conv.py

# Run the jobs
OUTDIR=${BASEDIR}/data/${EXP}_pp/${GRANULARITY}
mkdir -p ${OUTDIR}/sfcwind_conv_latlon
sbatch <<EOF
#!/bin/bash
...
#SBATCH --job-name="calc_sfcwind_conv"
#SBATCH --output=LOG/calc_sfcwind_conv.%j.out
#SBATCH --error=LOG/calc_sfcwind_conv.%j.out
#SBATCH --export=ALL
