...
RUNFILE=${BASEDIR}/scripts/calc_clwvi.py

# Run the jobs
OUTDIR=${BASEDIR}/data/${EXP}_pp/${GRANULARITY}
mkdir -p ${OUTDIR}/clwvi_latlon
sbatch <<EOF
#!/bin/bash
...
#SBATCH --job-name="calc_clwvi"
#SBATCH --output=LOG/calc_clwvi.%j.out
#SBATCH --error=LOG/calc_clwvi.%j.out
#SBATCH --export=ALL
