...
RUNFILE=${BASEDIR}/scripts/calc_sfc_density.py

# Run the jobs
OUTDIR=${BASEDIR}/data/${EXP}_pp/${GRANULARITY}
mkdir -p ${OUTDIR}/sfc_rho_latlon
sbatch <<EOF
#!/bin/bash
...
#SBATCH --job-name="calc_sfc_density"
#SBATCH --output=LOG/calc_sfc_density.%j.out
#SBATCH --error=LOG/calc_sfc_density.%j.out
#SBATCH --export=ALL
